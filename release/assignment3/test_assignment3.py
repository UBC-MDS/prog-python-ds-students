...
def test_1a(answer):
    sol = ['86f7e437faa5a7fce15d1ddcb9eaeaea377667b8', 'e9d71f5ee7c92d6dc9e92ffdad17b8bd49418f98']
    correct = 0
    for a in set(answer):
        correct += sha1(str(a).lower().encode("utf8")).hexdigest() in sol
    assert correct == 2, "You have {0} correct answer(s) and {1} incorrect answer(s).".format(correct, len(set(answer))-correct)
    return("Success")
...
def test_2h(answer):
    ...
    assert str(answer.encoding.x.shorthand).startswith('School') or str(answer.encoding.x.field) == 'School', "Make sure you are plotting the \
                                                                                                    `School` variable on the x-axis."
    assert str(answer.encoding.y.shorthand).startswith('Wage_Gap') or str(answer.encoding.y.field) == 'Wage_Gap', "Make sure you are plotting the \
...
def test_4i(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    assert (answer.mark == 'bar' or answer.mark.type == 'bar'), "Make sure you are using the 'mark_bar()' function"
    assert str(answer.encoding.x.shorthand).startswith('Continent') or str(answer.encoding.x.field) == 'Continent', "Make sure you are plotting the \
                                                                                                    `Continent` variable on the x-axis."
    assert str(answer.encoding.y.shorthand).startswith('Emission_pp') or str(answer.encoding.y.field) == 'Emission_pp', "Make sure you are plotting the \
